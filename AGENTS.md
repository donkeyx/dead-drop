...
- CI (PR + master) is tests and helm lint only. A `v*` tag on master publishes image + chart, then deploys; **Run workflow** on Release redeploys that tag. Chart is 0.1.6. No image publish from master. If HTMX is added later for non-sensitive chrome, it must never submit plaintext, fragment keys, or passphrases.
