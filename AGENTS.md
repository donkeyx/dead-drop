...
- `GET /api/v1/stats` is coarse durable totals only (created, burned, expired, passphrase). No ids, rates, or waiting counts. Increment in the same store transaction as the event.
...
- CI (PR + master) is tests and helm lint only. A `v*` tag on master publishes image + chart, then deploys; **Run workflow** on Release redeploys that tag. Chart is 0.1.9. No image publish from master. If HTMX is added later for non-sensitive chrome, it must never submit plaintext, fragment keys, or passphrases.
