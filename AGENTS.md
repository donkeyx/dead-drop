...
- `store/` owns filesystem and SQLite persistence. HTTP download must use atomic `Store.Take`, never `Get` followed by conditional delete.
...
- v1 is single-writer/single-node against a local data directory. The process lock and SQLite/FS atomicity do not support multiple replicas sharing storage.
