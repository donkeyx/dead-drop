...
A `v*` tag on `master` publishes the multi-arch image and OCI chart, then deploys. **Run workflow** redeploys an existing semver (`0.1.5` / `v0.1.5`) without rebuilding. Master and PRs only run CI — no image, no deploy. The job checks out `v<version>` so the chart matches the image.
